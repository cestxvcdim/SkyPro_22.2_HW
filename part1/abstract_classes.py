...
class Person:
    ...
    @staticmethod
    def use_transport(type_of_transport: Car | Boat | ElectroScooter):
        type_of_transport.start_engine()
        type_of_transport.move()
        type_of_transport.stop()
        type_of_transport.stop_engine()

# Отрезок кода для самопроверки.
# Запустите его, после того как выполните задание

if __name__ == '__main__':
    boat, car, kamikadze, person = Boat(), Car(), ElectroScooter(), Person()
